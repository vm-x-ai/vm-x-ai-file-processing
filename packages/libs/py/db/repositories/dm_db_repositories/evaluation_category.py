...
class EvaluationCategoryRepository(
    BaseRepository[
        UUID,
        dm_db_models.EvaluationCategory,
        dm_db_models.EvaluationCategoryRead,
        dm_db_models.EvaluationCategoryCreate,
    ]
):
    def __init__(
        self,
    ...
    def _id_fields(self) -> tuple[Column[UUID]]:
        return (cast(Column[UUID], dm_db_models.EvaluationCategory.id),)

    def _id_predicate(self, id: UUID) -> ColumnExpressionArgument[bool]:
        return col(dm_db_models.EvaluationCategory.id) == id
    ...
    async def get_by_project_id(
        ...
        async with self._session_factory() as session:
            query = (
                select(dm_db_models.EvaluationCategory)
                .where(dm_db_models.EvaluationCategory.project_id == project_id)
                .order_by(col(dm_db_models.EvaluationCategory.name))
            )
            result = await session.scalars(query)
            return [
                dm_db_models.EvaluationCategoryRead.model_validate(category)
                for category in result.all()
            ]

    async def get_by_name_and_project(
        self, name: str, project_id: UUID
    ) -> dm_db_models.EvaluationCategoryRead | None:
        """Get an evaluation category by name within a specific project."""
        async with self._session_factory() as session:
            query = select(dm_db_models.EvaluationCategory).where(
                dm_db_models.EvaluationCategory.name == name,
                dm_db_models.EvaluationCategory.project_id == project_id,
            )
            result = await session.scalars(query)
    ...
    async def create_default_category(
        ...
        default_category = dm_db_models.EvaluationCategoryCreate(
            id=uuid4(),
            name="default",
    ...
    async def find_or_create_by_name(
        ...
        new_category = dm_db_models.EvaluationCategoryCreate(
            id=uuid4(),
            name=name,
