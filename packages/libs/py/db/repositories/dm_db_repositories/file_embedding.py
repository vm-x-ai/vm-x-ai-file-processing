...
class FileEmbeddingRepository(
    BaseRepository[
        UUID,
        dm_db_models.FileEmbedding,
        dm_db_models.FileEmbeddingRead,
    ...
    def _id_fields(self) -> tuple[Column[UUID]]:
        return (cast(Column[UUID], dm_db_models.FileEmbedding.id),)

    def _id_predicate(self, id: UUID) -> ColumnExpressionArgument[bool]:
        return col(dm_db_models.FileEmbedding.id) == id

    async def get_by_file_id(
        self, file_id: UUID
    ) -> list[dm_db_models.FileEmbeddingRead]:
        async with self._session_factory() as session:
            query = select(dm_db_models.FileEmbedding).where(
                dm_db_models.FileEmbedding.file_id == file_id
            )

            result = await session.scalars(query)

            return [
                dm_db_models.FileEmbeddingRead.model_validate(embedding)
    ...
    def _create_base_similarity_search_query(
        ...
        filter_fn: Callable[
            [
                SelectOfScalar[tuple[dm_db_models.FileEmbedding, float]],
                Subquery,
            ],
            SelectOfScalar[tuple[dm_db_models.FileEmbedding, float]],
        ],
    ) -> (
        Select[tuple[dm_db_models.FileEmbedding, float, bool, UUID]]
        | Select[
            tuple[
                dm_db_models.FileContent, float, bool, UUID, dm_db_models.FileEmbedding
            ]
        ]
    ):
        ...
        """
        similarity_score = 1 - dm_db_models.FileEmbedding.embedding.cosine_distance(
            query_embedding
        )

        score_query = select(
            dm_db_models.FileEmbedding, similarity_score.label("score")
        )
        ...
        query: SelectOfScalar[tuple[dm_db_models.FileEmbedding, float]] = select(
            score_subquery
        )  # type: ignore
        ...
        query_cte_alias = aliased(dm_db_models.FileEmbedding, query_cte)
        neighbor_query: (
            Select[tuple[dm_db_models.FileEmbedding, Any]]
            | Select[tuple[dm_db_models.FileContent, Any]]
        )
        ...
        match payload.when_match_return:
            case SimilaritySearchWhenMatchReturn.CHUNK:
                neighbor_query = (
                    select(
                        dm_db_models.FileEmbedding,
                        func.array_agg(col(dm_db_models.FileEmbedding.id))
                        .over(
                            partition_by=col(dm_db_models.FileEmbedding.file_id),
                            order_by=col(dm_db_models.FileEmbedding.chunk_number),
                            range_=(
                    ...
                    .where(
                        col(dm_db_models.FileEmbedding.file_id)
            ...
            case SimilaritySearchWhenMatchReturn.CONTENT:
                neighbor_query = (
                    select(
                        ...
                        func.array_agg(col(dm_db_models.FileContent.id))
                        .over(
                            partition_by=col(dm_db_models.FileContent.file_id),
                            order_by=col(dm_db_models.FileContent.content_number),
                            range_=(
                                -payload.before_neighbor_count,
                                payload.after_neighbor_count,
                            ),
                    ...
                    .where(
                        col(dm_db_models.FileContent.file_id)
                        == col(query_cte.c.file_id),
                    )
                )
                ...
                result_query = (
                    (
                        select(
                            ...
                            case(
                                (
                                    col(dm_db_models.FileEmbedding.id)
                        ...
                        .order_by(
                            col(dm_db_models.FileEmbedding.chunk_number).asc()
                    ...
                    or payload.after_neighbor_count > 0
                        ...
                        .join(
                            dm_db_models.FileEmbedding,
                            col(dm_db_models.FileEmbedding.id) == col(query_cte.c.id),
                        )
                    )
                )
                ...
                result_query = (
                    (
                        ...
                        select(  # type: ignore
                            ...
                            case(
                                (
                                    col(dm_db_models.FileContent.id)
                                    != col(neighbor_query_cte.c.id),
                                    literal(0),
                                ),
                                else_=query_cte.c.score,
    ...
    def _parse_similarity_search_result(
        ...
        payload: SimilaritySearchRequest,
        db_results: (
            Sequence[tuple[dm_db_models.FileEmbedding, float, bool, UUID]]
            | Sequence[
                tuple[
                    dm_db_models.FileContent,
                    float,
        ...
        """Parses raw similarity search results into FileEmbeddingRead dm_db_models.
    ...
    def _parse_chunk_result(
        self,
        db_results: Sequence[tuple[dm_db_models.FileEmbedding, float, bool, UUID]],
    ) -> list[dm_db_models.FileEmbeddingRead]:
        """
        ...
        FileEmbeddingRead dm_db_models.
        ...
        """
        file_chunk_map = {
            chunk.id: dm_db_models.FileEmbeddingRead(
                **chunk.model_dump(),
        ...
        chunk_result: list[dm_db_models.FileEmbeddingRead] = []
    ...
    def _parse_content_result(
        ...
        Parses the Similarity Search Query result into a list of \
            FileContentReadWithChunkScore dm_db_models.
        ...
        """
        file_chunk_map: dict[UUID, dm_db_models.FileEmbeddingRead] = {}
        root_file_content_map, file_content_map = self._map_content_result(db_results)

        content_result: list[dm_db_models.FileContentReadWithChunkScore] = []
        for content, score, is_neighbor, neighbor_from, chunk in db_results:
            ...
            if chunk_id not in file_chunk_map and chunk.content_id == content_id:
                file_chunk_map[chunk_id] = dm_db_models.FileEmbeddingRead(
                    **chunk.model_dump(),
                    score=score,
                )
    ...
    def _map_content_result(
        ...
        dict[UUID, dm_db_models.FileContentReadWithChunkScore],
        dict[UUID, dm_db_models.FileContentReadWithChunkScore],
    ]:
        """
        ...
        root_file_content_map: dict[
            UUID, dm_db_models.FileContentReadWithChunkScore
        ] = {}
        file_content_map: dict[UUID, dm_db_models.FileContentReadWithChunkScore] = {}
        ...
        for content, _, is_neighbor, _, _ in db_results:
            ...
            item_content = dm_db_models.FileContentReadWithChunkScore(
                **content.model_dump(),
                match_chunks=[],
                before_neighbors=[],
                after_neighbors=[],
