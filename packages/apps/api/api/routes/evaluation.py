...
from internal_schemas.evaluation import (
...
@router.get(
    ...
    response_model=list[internal_db_models.EvaluationRead],
    tags=["evaluations"],
)
@inject
...
@router.get(
    ...
    description="Get all evaluations for a project",
    response_model=list[internal_db_models.EvaluationTree],
    tags=["evaluations"],
)
@inject
...
) -> list[internal_db_models.EvaluationTree]:
    evaluations = await evaluation_repository.get_by_project_id(
        project_id=project_id,
    )

    root_evaluations: list[internal_db_models.EvaluationTree] = []
    evaluation_map = {
        evaluation.id: internal_db_models.EvaluationTree.model_validate(evaluation)
...
async def get_files_by_evaluation(
    ...
    async def update_file_evaluation(
        file_evaluation: internal_db_models.FileEvaluationReadWithFile,
    ) -> internal_db_models.FileEvaluationReadWithFile:
...
@router.post(
    ...
    response_model=internal_db_models.EvaluationRead,
    tags=["evaluations"],
)
@inject
async def create_evaluation(
    ...
    evaluation = await evaluation_repository.add(
        internal_db_models.EvaluationCreate.model_validate(
            {
                **payload.model_dump(
...
@router.post(
    ...
    response_model=internal_db_models.EvaluationCategoryRead,
    tags=["evaluation-categories"],
)
@inject
async def create_evaluation_category(
    ...
    return await evaluation_category_repository.add(
        internal_db_models.EvaluationCategoryCreate.model_validate(
            {
                "name": payload["name"],
