...
from ingestion_workflow.schema.evaluation_template import (
...
@router.get(
    ...
    response_model=list[models.EvaluationTemplateRead],
...
@router.post(
    ...
    response_model=models.EvaluationTemplateRead,
    tags=["evaluation-templates"],
)
@inject
async def create_evaluation_template(
    ...
    return await evaluation_template_repository.add(
        models.EvaluationTemplateCreate.model_validate(
            {
                **payload.model_dump(),
                "project_id": project_id,
