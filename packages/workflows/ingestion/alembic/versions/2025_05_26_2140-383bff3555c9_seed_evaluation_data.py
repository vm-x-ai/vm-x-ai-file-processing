...
def upgrade() -> None:
    ...
    for project in projects:
        ...
        connection.execute(sa.text("""
            ...
            SELECT gen_random_uuid(), 'Diligence Questions', 'Category: My new category', :project_id, now(), now()
            WHERE NOT EXISTS (
        ...
        connection.execute(sa.text("""
            INSERT INTO evaluation_categories (id, name, description, project_id, created_at, updated_at)
            SELECT gen_random_uuid(), 'Legal Documents', 'Category for legal document evaluations', :project_id, now(), now()
            WHERE NOT EXISTS (
                SELECT 1 FROM evaluation_categories 
        ...
        diligence_cat_result = connection.execute(sa.text("""
            SELECT id FROM evaluation_categories 
            WHERE name = 'Diligence Questions' AND project_id = :project_id
        """), {"project_id": project_id})
        diligence_cat_id = diligence_cat_result.fetchone()[0]
        
        legal_cat_result = connection.execute(sa.text("""
            SELECT id FROM evaluation_categories 
            WHERE name = 'Legal Documents' AND project_id = :project_id
        """), {"project_id": project_id})
        ...
        evaluations = [
            {
                ...
                'system_prompt': "You are an evaluator of company documents. Your job is to determine if the content you're evaluating contains a mention of the CEO of the company for this doc.",
            ...
            },
            {
                ...
                'system_prompt': "YOu check if the CFO is mentioned. Evaluate if they were mentioned in the doc",
                'category_id': diligence_cat_id
            },
            {
                ...
                'system_prompt': "You are a document analyzier. You check if the document page or snippet that you've bene presented with contains financial projections or not.",
                'category_id': diligence_cat_id
            },
            {
                ...
                'system_prompt': "You're a document evaluator. You determine if any forward looking statements were mentioned in this document. You get 1 page or 1 snippet at a time, and determine if it has such a statement",
                'category_id': diligence_cat_id
            },
            {
                'title': 'HR contracts',
                ...
                'system_prompt': "You check if there's any mention of an HR contracts in a document. You get a document snippet and evaluate it.",
                'category_id': diligence_cat_id
            },
            {
                ...
                'system_prompt': "You evaluate documents. You get one snippet at a time. The quetsion you're evaluating is if any Procurement contracts are mentioned.",
        ...
        for eval_data in evaluations:
            connection.execute(sa.text("""
                ...
                SELECT 
                    gen_random_uuid(), :title, :description, :evaluation_type, :evaluation_options,
                    :prompt, :system_prompt, :project_id, :category_id,
                    NULL, NULL, now(), now()
                WHERE NOT EXISTS (
...
def downgrade() -> None:
    ...
    evaluation_titles = [
        'CEO mentions', 'CFO mention', 'Financial Projections', 
        'Forward looking statements', 'HR contracts', 'Procurement contracts'
    ]
